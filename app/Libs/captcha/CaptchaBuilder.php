...
/**
 * Minho / php-captcha  https://gitee.com/longfei6671/php-captcha
 * Created by PhpStorm.
 * user: lifeilin
 * Date: 2017/1/11 0011
 * Time: 10:19
...
class CaptchaBuilder
{
     ...
     * @var string 随机字符
     */
    protected $characters = '0123456789abcdefghjmnpqrtuxyzABCDEFGHJMNPQRTUXYZ';
    /**
     * @var int 图片宽度
     */
    protected $width = 150;
    ...
    public function initialize(array $config)
    {
        ...
        $this->number = isset($config['count']) ? $config['count'] : 4;
        ...
        isset($config['snowflake']) && $this->isDrawNoise = boolval($config['snowflake']);
        if (isset($config['fonts']) && empty($config['fonts']) === false) {
            $this->fonts = './font/' . $config['fonts'];
        } else {
            $fontDir = './font/';
            ...
            $this->fonts = $fonts[array_rand($fonts)];
        }
    ...
    public function create()
    {
        ...
        setcookie("font", $this->fonts);
        $this->textFont = $this->fonts;
    ...
    {
        $colors[0] = 200 + mt_rand(1, 54);
        $colors[1] = 200 + mt_rand(1, 54);
        $colors[2] = 200 + mt_rand(1, 54);
        return $colors;
    }
