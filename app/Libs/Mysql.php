...
class MMysql
{
    ...
    public function __construct(array $conf)
    {
        ...
        $this->_host = $conf['SQL'];
        $this->_port = $conf['SQLp'];
        $this->_user = $conf['SQLu'];
        $this->_pass = $conf['SQLs'];
        $this->_dbName = $conf['SQLb'];
        //连接数据库
    ...
    public function inserts($tbName, array $keys, array $data)
    {
        $data = $this->_dataFormat($tbName, $data);
        if (!$data) return;
        $values = "";
        ...
        $sql = "insert into " . $tbName . "(" . implode(',', $keys) . ") values " . $values;
        return $this->_doExec($sql);
    }
    ...
    public function countPhone($tbName)
    {
        $sql = "SELECT COUNT(*) AS totalCount,count(if(code=0,code,NULL)) AS dis,count(if(code=1,code,NULL)) AS line,count(if(time=0,time,NULL)) AS noline FROM " . $tbName;
        $this->_clear = 1;
        $this->_clear();
        return $this->_doQuery($sql);
    }
    ...
    {
        $sql = "SELECT COUNT(*) AS totalCount FROM " . $tbName . " " . $this->_where;
        $this->_clear = 1;
        $this->_clear();
        return $this->_doQuery($sql);
    }
